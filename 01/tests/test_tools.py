...
from agents.general_chat import search_knowledge


@pytest.mark.asyncio
@patch("utils.get_vector_store")  # ✅ 拦截 utils.get_vector_store
async def test_search_knowledge_success(mock_get_store):
    # 1. 构造 Mock 链条
    ...
    # 2. 执行测试
    result = await search_knowledge.ainvoke({"query": "测试"})
...
async def test_search_knowledge_empty(mock_get_store):
    # 模拟检索结果为空
    mock_store_instance = MagicMock()
    mock_retriever = AsyncMock()
    ...
    result = await search_knowledge.ainvoke({"query": "火星移民"})
    assert "未在知识库中找到相关规定" in result

@pytest.mark.asyncio
@patch("utils.get_vector_store")
async def test_search_knowledge_db_error(mock_get_store):
    # 模拟 get_vector_store 返回 None (初始化失败)
    mock_get_store.return_value = None
    
    result = await search_knowledge.ainvoke({"query": "随便问问"})
